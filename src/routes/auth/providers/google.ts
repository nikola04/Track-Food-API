...
router.post("/", async (req: Request, res: Response) => {
    ...
    try {
        ...
        TokenModel.create({ user_id: account.user_id, hashed_token: hashedToken, user_agent: userAgent, expires_at: refreshExpiry }); // dont wait to create
        const ip = req.device.ip;
        const location = await getIpLocation(ip);
        LoginModel.create({ user_id: account.user_id, hashed_token: hashedToken, ip, location });
